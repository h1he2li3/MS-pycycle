...
class Properties(object):
    ...
    def __init__(self, thermo_data_module, init_elements=None):
        ...
        if isinstance(self.thermo_data_module, bool):
            # Handle the case when it is a boolean object
            # Perform appropriate actions or raise an exception
            ...
            for element in self.elements:
                if element not in valid_elements:
                    if element in self.prod_data.keys():
                        raise ValueError(
                            f"The provided element `{element}` is a product in your provided thermo data, but is not an element."
                        )
                    else:
                        raise ValueError(
                            f"The provided element `{element}` is not used in any products in your thermo data."
                        )
        ...
        self.build_coeff_table(
            999
        )  # just pick arbitrary default temperature so there is something there right away

    def H0(self, Tt):  # standard-state molar enthalpy for species j at temp T
        Tt = Tt[0]
        if (
            Tt < self.valid_temp_range[0] or Tt > self.valid_temp_range[1]
        ):  # runs if temperature is outside range of current coefficients
