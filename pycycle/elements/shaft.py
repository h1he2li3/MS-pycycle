...
class Shaft(ExplicitComponent):
    ...
    def initialize(self):
        self.options.declare(
            "num_ports", default=2, desc="number shaft connections to make"
        )
    ...
    def setup(self):
        ...
        # Donot know where this value comes from
        self.add_input("HPX", val=0.0, units="hp", desc="shaft power")
        self.add_input("fracLoss", val=0.0, desc="loss fraction")
        ...
        self.convert = 2.0 * np.pi / 60.0 / HP_to_FT_LBF_per_SEC

        self.trq_vars = []
        for i in range(num_ports):
            trq_var_name = "trq_{:d}".format(i)
            self.add_input(trq_var_name, val=0.0, units="ft*lbf")

            self.trq_vars.append(trq_var_name)

            self.declare_partials(
                ["trq_in", "trq_out", "pwr_in", "pwr_out"], trq_var_name
            )
    ...
    def compute(self, inputs, outputs):
        ...
        # If the torque is positive then the value is added to trq_in, otherwise it is added to trq_out
        ...
        # Net torque equals remaining input torque after loss fraction + output torque - shaft torque (after unit conversion and conversion of power to torque by dividing by shaft rotation speed or Nmech)
        trq_net = trq_in * (1.0 - fracLoss) + trq_out - HPX / (Nmech * self.convert)
        ...
        outputs["pwr_in_real"] = trq_in * (1.0 - fracLoss) * Nmech * self.convert
    ...
    def compute_partials(self, inputs, J):
        ...
        J["trq_net", "Nmech"] = HPX * Nmech ** (-2.0) / self.convert
        J["trq_net", "HPX"] = -1.0 / (Nmech * self.convert)
        ...
        J["pwr_net", "Nmech"] = (
            trq_in * (1 - fracLoss) * self.convert + trq_out * self.convert
        )
        J["pwr_net", "HPX"] = -1
