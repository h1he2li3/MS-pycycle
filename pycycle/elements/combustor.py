...
class Combustor(Element):
    ...
    -------------
        inputs
        --------
        Fl_I:FAR or Input fuel and air mixture
        dPqP or
        MN
    ...
    def setup(self):
        ...
        vit_flow = Thermo(
            ...
            thermo_kwargs={"composition": air_fuel_composition, "spec": thermo_data},
        )
...
if __name__ == "__main__":
    ...
    p.model.add_subsystem(
        "d1",
        om.IndepVarComp("Fl_I:stat:W", val=1.0, units="lbm/s", desc="weight flow"),
        promotes=["*"],
    )
    p.model.add_subsystem(
        "d2",
        om.IndepVarComp("Fl_I:FAR", val=0.2, desc="Fuel to air ratio"),
        promotes=["*"],
    )
    p.model.add_subsystem(
        "d3",
        om.IndepVarComp("Fl_I:tot:h", val=1.0, units="Btu/lbm", desc="total enthalpy"),
        promotes=["*"],
    )
    p.model.add_subsystem(
        "d4",
        om.IndepVarComp("fuel_Tt", val=518.0, units="degR", desc="fuel temperature"),
        promotes=["*"],
    )
