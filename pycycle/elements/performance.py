...
class Performance(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            ...
            val=14.696,
            units="lbf/inch**2",
            desc="pressure at the inlet of the first compressor",
        )
        self.add_input(
            "Pt3",
            val=14.696,
            units="lbf/inch**2",
            desc="pressure at the exit of the last compressor",
        )
        ...
        # Gross thrust values from 'x' nozzles is created first as an empty list.
        self.Fg_vals = []
        for i in range(num_nozzles):
            ...
            self.add_input(
                ...
                val=0.0,
                units="lbf",
                desc="gross thrust from nozzle {:d}".format(i),
            )
            # self.add_input(Fg_0, val=0., units='lbf', desc='gross thrust from nozzle 0')
            self.Fg_vals.append(Fg_val_name)
            # Fg_vals = ['Fg_0', 'Fg_1', 'Fg_2'] in case of multiple or 3 nozzles

        # Similar to num_nozzles part
        ...
        for i in range(num_burners):
            ...
            self.add_input(
                ...
                val=0.0,
                units="lbm/s",
                desc="fuel flow rate entering combustor {:d}".format(i),
            )
        ...
        self.add_output(
            "Fg", val=10000.0, units="lbf", desc="gross thrust of all nozzles"
        )
        ...
        if num_burners > 0:
            ...
            self.add_output(
                ...
                val=0.001,
                units="lbm/s",
                desc="mass flow rate of fuel to combustor",
            )
    ...
    def compute(self, inputs, outputs):
        ...
        # Initializes Fg at 0, and then to the Fg value the gross thrust of each of the nozzles is all added up.
        Fg = 0.0
        for Fg_val in self.Fg_vals:
            Fg += inputs[Fg_val]
        outputs["Fn"] = Fn = Fg - inputs["ram_drag"]  # Net thrust from all the nozzles
        outputs["Fg"] = Fg  # Gross thrust of all the nozzles
        ...
        if self.Wfuel_vals:
            ...
            outputs["Wfuel"] = Wfuel
            outputs["TSFC"] = (
                Wfuel * 3600.0 / (Fn + 1e-10)
            )  # 1e-10 added to avoid zero division error.
            outputs["PSFC"] = Wfuel * 3600.0 / inputs["power"]
...
if __name__ == "__main__":
    ...
    # comp here means component i believe not the compressor but i could be wrong.
