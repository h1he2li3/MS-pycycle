...
class MilSpecRecovery(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        # outputs
        self.add_output(
            "ram_recovery", val=1.0, units=None, desc="Mil Spec Ram Recovery"
        )
...
class Calcs(om.ExplicitComponent):
    ...
    def setup(self):
        # inputs
        self.add_input(
            "Pt_in", val=5.0, units="lbf/inch**2", desc="Entrance total pressure"
        )
        ...
        # outputs
        self.add_output(
            "Pt_out", val=14.696, units="lbf/inch**2", desc="Exit total pressure"
        )
...
class Inlet(Element):
    ...
    def setup(self):
        ...
        # composition = self.Fl_I_data.get("Fl_I", default_value=None)
        ...
        if statics:
            ...
            self.add_subsystem(
                "W_passthru",
                PassThrough("Fl_I:stat:W", "Fl_O:stat:W", 0.0, units="lbm/s"),
                promotes=["*"],
            )
    ...
    def pyc_setup_thermo(self, upstream):
        ...
        self.Fl_O_data = {"Fl_O": upstream["Fl_I"]}
...
# # Create an instance of the Inlet class
# inlet = Inlet()

# # Add the Inlet component to the Group
# prob.model.add_subsystem("inlet", inlet)

# # Set the design inputs for the Inlet
# prob.model.set_input_defaults("inlet.ram_recovery", val=0.95)
# prob.model.set_input_defaults("inlet.MN", val=0.6)

# # Set other necessary inputs and outputs for the Inlet

# # Setup the problem and run the model
# prob.setup()
# prob.run_model()
