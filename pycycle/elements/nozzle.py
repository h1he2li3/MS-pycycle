...
class PR_bal(om.ImplicitComponent):
    ...
    """
    Balance which varies the nozzle pressure ratio (PR) until the calculated exhaust static pressure (Ps_calc) matches
    the inputs exhaust static pressure (Ps_exhaust).  This balance is needed to handle situations where the nozzle
    pressure ratio or (Pt/Ps) drops below 1.
    """
...
class PressureCalcs(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        outputs["Pt_th"] = inputs["Pt_in"] * (1.0 - inputs["dPqP"])
        outputs["Ps_calc"] = outputs["Pt_th"] / inputs["PR"]

    def compute_partials(self, inputs, J):
        J["Pt_th", "Pt_in"] = 1.0 - inputs["dPqP"]
        J["Pt_th", "dPqP"] = -inputs["Pt_in"]
        J["Ps_calc", "Pt_in"] = (1.0 - inputs["dPqP"]) / inputs["PR"]
        J["Ps_calc", "PR"] = (-inputs["Pt_in"] * (1.0 - inputs["dPqP"]) * inputs["PR"] ** -2.0)
...
class PerformanceCalcs(om.ExplicitComponent):
    ...
    def initialize(self):
        self.options.declare("lossCoef", default="Cfg", desc='If set to "Cfg", then Gross Thrust Coefficient is an input.',)
    ...
    def compute(self, inputs, outputs):
        ...
        if lossCoef == "Cfg":
            ...
            outputs["Fg"] = (inputs["W_in"] / g_c) * inputs["V_actual"] * inputs["Cv"] * inputs["Cang"] * inputs["CmixCorr"] + (inputs["Ps_actual"] - inputs["Ps_calc"]) * inputs["A_actual"]

    def compute_partials(self, inputs, J):
        lossCoef = self.options["lossCoef"]

        J["Fg_ideal", "W_in"] = 1.0 / g_c * inputs["V_ideal"]
        J["Fg_ideal", "V_ideal"] = inputs["W_in"] / g_c

        if lossCoef == "Cfg":
            J["Fg", "W_in"] = 1.0 / g_c * inputs["V_ideal"] * inputs["Cfg"]
            J["Fg", "V_ideal"] = inputs["W_in"] / g_c * inputs["Cfg"]
            J["Fg", "Cfg"] = (inputs["W_in"] / g_c) * inputs["V_ideal"]
        else:
            J["Fg", "W_in"] = (1.0 / g_c * inputs["V_actual"] * inputs["Cv"] * inputs["Cang"] * inputs["CmixCorr"])
...
class Mux(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare("fl_out_name", default="Fl_O", desc="Outflow station prefix.")
    ...
    def compute_partials(self, inputs, J):
        ...
        if nozzType == "CV":
            ...
            for p in self.flow_out:
                ...
                J["%s:stat:%s" % (fl_out_name, p), "%s:%s" % (prefix, p)] = 1.0

                J["Throat:stat:%s" % p, "%s:%s" % (other, p)] = 0.0
                J["%s:stat:%s" % (fl_out_name, p), "%s:%s" % (other, p)] = 0.0
...
class Nozzle(Element):
    """
    An assembly that models a convergent Nozzle as a pyCycle Element.
    Nozzle uses few other Balance, Implicit and Explicit components to achieve the goal.
    """
    ...
    def setup(self):
        ...
        self.add_subsystem("press_calcs", PressureCalcs(), promotes_inputs=prom_in, promotes_outputs=["Ps_calc"])
        ...
        prom_in = [("h", "Fl_I:tot:h"), ("composition", "Fl_I:tot:composition")]
        self.add_subsystem("throat_total", throat_total, promotes_inputs=prom_in, promotes_outputs=["Fl_O:*"])
        self.connect("press_calcs.Pt_th", "throat_total.P")

        # Calculate static properties for sonic flow
        throat_static_MN = Thermo(mode="static_MN", method=thermo_method, thermo_kwargs={"composition": composition, "spec": thermo_data})
        prom_in = [("ht", "Fl_I:tot:h"), ("W", "Fl_I:stat:W"), ("composition", "Fl_I:tot:composition")]
        self.add_subsystem("staticMN", throat_static_MN, promotes_inputs=prom_in)
        ...
        throat_static_Ps = Thermo(mode="static_Ps", method=thermo_method, thermo_kwargs={"composition": composition, "spec": thermo_data})
        prom_in = [("ht", "Fl_I:tot:h"), ("W", "Fl_I:stat:W"), ("Ps", "Ps_calc"), ("composition", "Fl_I:tot:composition")]
        self.add_subsystem("staticPs", throat_static_Ps, promotes_inputs=prom_in)
        ...
        prom_in = [("ht", "Fl_I:tot:h"), ("S", "Fl_I:tot:S"), ("W", "Fl_I:stat:W"), ("Ps", "Ps_calc"), ("composition", "Fl_I:tot:composition")]
        ...
        prom_in = [("Ps:W", "Fl_I:stat:W"), ("MN:W", "Fl_I:stat:W"), ("Ps:P", "Ps_calc"), "Ps_calc"]
        ...
        self.add_subsystem("perf_calcs", perf_calcs, promotes_inputs=prom_in, promotes_outputs=["Fg"])
